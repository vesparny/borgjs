  ...
  return function sendPushNotification ({ subject, message }, cb) {
    getDevices((err, res, body) => {
      if (err || res.statusCode !== 200) {
        console.log(
          chalk.red('Error while retrieving devices for push notification.'),
          err
        )
        return cb()
      }
          ...
          return function (cb) {
            sendPush(
              ...
              (err, res, body) => {
                if (err || res.statusCode !== 200) {
                  console.log(
                    chalk.red('Error while sending push notification.'),
                    err
                  )
                } else {
                  console.log(
                    chalk.yellow(`Push notification sent to ${el.nickname}.`)
                  )
                }
                cb()
              }
